...
export default function CheckoutPage() {
  ...
  // Prevent the existing "empty cart => /cart" redirect from firing
  // immediately after a successful checkout when clearCart() runs.
  ...
  useEffect(() => {
    ...
    // not after a successful submission that cleared the cart.
    if (!hasSubmitted && items.length === 0 && !isLoading) router.push("/cart");
  ...
  }, [user, isLoading, items.length, router, hasSubmitted]);
  ...
  const buildShippingAddress = () => {
    // Order Service expects shippingAddress as a string.
    const parts = [
  ...
  const payForOrder = async (orderId: string | number) => {
    ...
    return (await res.json()) as Promise<{ clientSecret: string }>;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect user to order history instead of cart/success page
